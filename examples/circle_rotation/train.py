...
def _save_yaml(cfg: SimpleNamespace, path: str) -> None:
...
def _invariance_loss(
    ...
    """Relative Frobenius error enforcing invariance of the learned basis under the operator."""
    Lt = (A_hat @ basis.T).T
...
def train(
    *, cfg_path: str = "config.yaml", save_dir: str = "./checkpoints", **override: Any
    ...
    ntr, nva, nte = cfg.ntrain, cfg.nvalid, cfg.ntest
    x_tr, x_va, x_te = np.split(x, [ntr, ntr + nva])
    y_tr, y_va, y_te = np.split(y, [ntr, ntr + nva])
    ...
    # surrogate operator
    with torch.no_grad():
        F = x_tr.reshape(ntr, -1).T
        LF = y_tr.reshape(ntr, -1).T
    ...
    for ep in range(cfg.epochs):
        model.train()
        t0 = time.time()
        tr_tot = tr_op = tr_in = tr_out = tr_inv = 0.0
        ...
        for xb, yb in tr_loader:
            ...
            in_loss = rel_loss(a_in, xb)
            out_loss = rel_loss(a_out, yb)
            sp_loss = torch.norm(bases, 1, 1).mean()

            loss = (
                cfg.lambda_op * op_loss
                + cfg.lambda_in * in_loss
                + cfg.lambda_out * out_loss
        ...
        model.eval()
        va_tot = va_op = va_in = va_out = va_inv = 0.0
        with torch.no_grad():
        ...
        logger.log(
            ...
            f"train_tot:{tr_tot:.7f} | train_op:{tr_op:.7f} | train_aec_in:{tr_in:.7f} | "
            f"train_aec_out:{tr_out:.7f} | train_inv:{tr_inv:.7f} | "
            f"valid_tot:{va_tot:.7f} | valid_op:{va_op:.7f} | valid_aec_in:{va_in:.7f} | "
            f"valid_aec_out:{va_out:.7f} | valid_inv:{va_inv:.7f} | nbasis:{model.nbasis}"
        )
